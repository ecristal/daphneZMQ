...
today_367_hd() {
  ...
  if [[ -n "$HD_367_CHARGE_WINDOWS_JSON" ]]; then
    run_client client/dynamic_range_led/scan_led_intensity_charge.py \
      --output-dir client/dynamic_range_led/runs/led_intensity_mask1010_367nm_2000_4000_L1024_cfg2100_off1450_w"${NEW_367_TICKS}"_retake \
...
today_367_vd() {
  ...
  if [[ -n "$VD_367_CHARGE_WINDOWS_JSON" ]]; then
    run_client client/dynamic_range_led/scan_led_intensity_charge.py \
      --output-dir client/dynamic_range_led/runs/led_intensity_mask1011_367nm_0000_4000_L1024_cfg2600_off0800_ch4567_retake \
...
today_270() {
  ...
  for vgain in 500 1200 2100; do
    ...
    run_client client/dynamic_range_led/scan_led_intensity_charge.py \
      --output-dir "client/dynamic_range_led/runs/led_intensity_charge_mask1_270nm_700_1750_L1024_5k_vgain${vgain}_off2200" \
...
Targets:
  today_367_hd    Retake mask1010 367 nm HD scan with fewer ticks than Mar 11
  today_367_vd    Retake mask1011 367 nm VD scan with 0:4000:50 and original ticks
  today_270       Retake VGAIN 500,1200,2100 270 nm runs with offset 2200
...
main() {
  ...
  case "$target" in
    today_367_hd) today_367_hd ;;
    today_367_vd) today_367_vd ;;
    today_270) today_270 ;;
    all)
      today_367_hd
      today_367_vd
