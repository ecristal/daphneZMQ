...
Usage:
  ...
  python3 configure_fe_min_v2.py 127.0.0.1 9876 --afe-bias-controlled-volts 0:48
"""
...
def main():
    ...
    ap.add_argument(
        "--afe-bias-controlled-volts",
        action="append",
        default=[],
        metavar="AFE:VOLT",
        help=(
            "Per-AFE controlled bias voltage target in volts. "
            "Uses the closed-loop controlled bias command after CONFIGURE_FE, "
            "e.g. --afe-bias-controlled-volts 0:48 --afe-bias-controlled-volts 1:52"
        ),
    )
    ap.add_argument(
        "--bias-tolerance-mv",
        type=int,
        default=20,
        help="Controlled AFE bias tolerance in mV, default 20.",
    )
    ap.add_argument(
        "--bias-settling-time-ms",
        type=int,
        default=500,
        help="Controlled AFE bias settling time in ms, default 500.",
    )
    ap.add_argument(
        "--bias-dwell-time-ms",
        type=int,
        default=20,
        help="Controlled AFE bias dwell time per iteration in ms, default 20.",
    )
    ap.add_argument(
        "--bias-samples-per-read",
        type=int,
        default=4,
        help="ADC samples averaged per controller iteration, default 4.",
    )
    ap.add_argument(
        "--bias-max-iterations",
        type=int,
        default=20,
        help="Maximum controlled bias iterations, default 20.",
    )
    ap.add_argument(
        "--bias-proportional-gain",
        type=float,
        default=0.75,
        help="Controlled bias proportional gain, default 0.75.",
    )
    ap.add_argument(
        "--bias-max-dac-step",
        type=int,
    ...
    afe_bias_controlled_targets: dict[int, float] = {}
    for afe, volts in _parse_afe_kv_tokens(
        args.afe_bias_controlled_volts,
        label="--afe-bias-controlled-volts",
    ):
        if volts < 0.0:
            raise ValueError(f"afe{afe} controlled bias voltage must be non-negative, got {volts}")
        afe_bias_controlled_targets[afe] = volts

    overlapping_afes = sorted(set(afe_bias_overrides) & set(afe_bias_controlled_targets))
    if overlapping_afes:
        print(
            "[warning] AFE(s) requested through both legacy bias write and controlled bias write: "
            + ", ".join(str(afe) for afe in overlapping_afes)
        )
        print("          The controlled write runs after the legacy write, so it determines the final setting.")
        print()
    ...
    controlled_bias_writes_json = []
    if afe_bias_controlled_targets:
        print("[CONTROLLED_BIAS_WRITEBACK]")

        if not hasattr(pb_low, "cmd_writeAFEBiasControlledSet"):
            print(
                "Generated low-level protobuf module does not contain "
                "cmd_writeAFEBiasControlledSet. Regenerate the Python protos after "
                "adding the controlled bias messages."
            )
            return 16

        if not hasattr(pb_low, "cmd_writeAFEBiasControlledSet_response"):
            print(
                "Generated low-level protobuf module does not contain "
                "cmd_writeAFEBiasControlledSet_response. Regenerate the Python protos after "
                "adding the controlled bias messages."
            )
            return 16

        if not hasattr(pb_high, "MT2_WRITE_AFE_BIAS_CONTROLLED_SET_REQ"):
            print(
                "Generated high-level protobuf module does not contain "
                "MT2_WRITE_AFE_BIAS_CONTROLLED_SET_REQ. Add the new message type and regenerate protos."
            )
            return 16

        if not hasattr(pb_high, "MT2_WRITE_AFE_BIAS_CONTROLLED_SET_RESP"):
            print(
                "Generated high-level protobuf module does not contain "
                "MT2_WRITE_AFE_BIAS_CONTROLLED_SET_RESP. Add the new response message type and regenerate protos."
            )
            return 16

        for afe_id, target_volts in sorted(afe_bias_controlled_targets.items()):
            wreq = pb_low.cmd_writeAFEBiasControlledSet()
            wreq.afe_block = afe_id
            wreq.target_bias_mv = int(round(target_volts * 1000.0))
            wreq.tolerance_mv = int(args.bias_tolerance_mv)
            wreq.settling_time_ms = int(args.bias_settling_time_ms)
            wreq.dwell_time_ms = int(args.bias_dwell_time_ms)
            wreq.samples_per_read = int(args.bias_samples_per_read)
            wreq.max_iterations = int(args.bias_max_iterations)
            ...
            wenv.version = 2
            wenv.dir = pb_high.DIR_REQUEST
            wenv.type = pb_high.MT2_WRITE_AFE_BIAS_CONTROLLED_SET_REQ
            wenv.payload = wreq.SerializeToString()
            wenv.task_id, wenv.msg_id = next_ids()
            wenv.timestamp_ns = time.time_ns()
            wenv.route = args.route
            ...
            tw_send = time.time_ns()
            print_envelope(f"CONTROLLED BIAS REQUEST AFE{afe_id}", wenv)
            try:
                sock.send(wenv.SerializeToString())
            except zmq.Again:
                print(f"[timeout] controlled bias send timed out for AFE{afe_id} after {args.timeout} ms")
                return 17
            ...
            try:
                ...
                print(f"[timeout] no controlled bias response for AFE{afe_id} after {args.timeout} ms")
                return 18
            ...
            wreply = pb_high.ControlEnvelopeV2()
            if not wreply.ParseFromString(wreply_bytes):
                print(f"Failed to parse controlled bias reply envelope for AFE{afe_id}")
                return 19

            print_envelope(f"CONTROLLED BIAS RESPONSE AFE{afe_id}", wreply)
            print(f"[CONTROLLED_BIAS_METRICS AFE{afe_id}]")
            print(f"  client_send_iso  : {ns_to_iso(tw_send)}")
            print(f"  client_recv_iso  : {ns_to_iso(tw_recv)}")
            print(f"  client_RTT_ms    : {tw_rtt_ms:.2f}")
            if wreply.timestamp_ns:
            ...
            if wreply.dir != pb_high.DIR_RESPONSE:
                print(f"Controlled bias dir mismatch for AFE{afe_id}: got {wreply.dir}")
                return 20

            if wreply.type != pb_high.MT2_WRITE_AFE_BIAS_CONTROLLED_SET_RESP:
                print(
                    f"Controlled bias type mismatch for AFE{afe_id}: "
                    f"got {wreply.type}, expected {pb_high.MT2_WRITE_AFE_BIAS_CONTROLLED_SET_RESP}"
                )
                return 21

            if wreply.task_id != wenv.task_id or wreply.correl_id != wenv.msg_id:
                print(
                    f"Controlled bias correlation mismatch for AFE{afe_id}: "
                    f"task {wreply.task_id}/{wenv.task_id}, correl {wreply.correl_id}/{wenv.msg_id}"
                )
                return 22

            wout = pb_low.cmd_writeAFEBiasControlledSet_response()
            if not wout.ParseFromString(wreply.payload):
                print(f"Failed to parse cmd_writeAFEBiasControlledSet_response for AFE{afe_id}")
                return 23
            ...
            print(
                ...
                f"best_error={wout.best_error_mv} mV "
                f"iterations={wout.iterations} "
                f"settled={wout.settled} "
                f"status={wout.status} "
                f"message={wout.message}"
            )

            controlled_bias_writes_json.append(
                {
                    "afe": afe_id,
                    "target_bias_mv": wreq.target_bias_mv,
                    "controller_request": {
                        "tolerance_mv": wreq.tolerance_mv,
                        "settling_time_ms": wreq.settling_time_ms,
                        "dwell_time_ms": wreq.dwell_time_ms,
                        "samples_per_read": wreq.samples_per_read,
                    ...
                    "controlled_bias_response": {
                        ...
                        "best_error_mv": wout.best_error_mv,
                        "iterations": wout.iterations,
                        "settled": wout.settled,
                        "status": int(wout.status),
                    },
                }
            )
        print()
    ...
    if args.json:
        ...
        if controlled_bias_writes_json:
            summary["controlled_bias_writes"] = controlled_bias_writes_json
