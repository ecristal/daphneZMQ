...
void run_router_server(zmq::context_t& ctx,
  ...
  while (true) {
    ...
    telemetry::BeginCommand(req, client_id);
    ...
    if (streaming_it != streaming_handlers.end()) {
      try {
        ...
        telemetry::CompleteCommand(req.msg_id(), false, e.what());
    ...
    if (it == handlers.end()) {
      ...
      telemetry::CompleteCommand(req.msg_id(), false, "No handler registered");
      const auto env = v2::make_response(req, v2::response_type(req.type()), std::string{});
      send_to(router, client_id, env.SerializeAsString());
      continue;
    }
    ...
    telemetry::CompleteCommand(req.msg_id(), true, "Handler completed");
